...
- **MRR (Mean Reciprocal Rank)**: 平均倒数排名，衡量第一个相关文档的排名质量
- **Mean Similarity Score**: 平均相似度分数（基于向量距离）
- **Top-K Accuracy**: Top-1、Top-3、Top-5准确率，衡量前K个结果中是否包含相关文档
...
- 跳过LLM生成步骤，只评估检索质量
- 不计算答案相似度等生成相关指标
- 添加检索专用指标（MRR、Top-K准确率等）
- 大幅提升评估速度，节省LLM API调用成本
...
- `predicted_answer`: 模型生成的答案（仅在完整RAG评估中存在）
- `retrieved_doc_ids`: 检索到的文档ID列表
- `expected_doc_ids`: 期望的文档ID列表
- `sources`: 检索到的文档片段详情，每个source包含：
  - `chunk_id`: 文档块ID
  - `title`: 文档标题
  - `doc_id`: 文档ID
  - `distance`: 相似度距离（越小越相似）
  - `chunk_content`: **文档块的实际文本内容**（便于分析召回质量）
- `metadata`: 元数据信息（检索数量、上下文长度等）

**检索专用模式** (`--retrieval-only`)：

- 不包含 `predicted_answer` 字段
- 专注于评估分片、索引和召回效果
- 提供更详细的检索指标（MRR、Top-K准确率等）
